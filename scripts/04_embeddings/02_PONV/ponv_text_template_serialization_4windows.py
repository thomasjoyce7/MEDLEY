# Script to serialize tabular PONV medication data to a text format using a text template
...
def text_template(tabular_input: pd.DataFrame, window: int):
    ...
    for subject_id, subj_df in tabular_input.groupby("subject_id", dropna=False):
        ...
        for _, row in group.iterrows():
            ...
            entry = (f"{rel_time_str}, the patient received {dose} {units} of {med} via {route} route.")
...
medication_texts_window1 = dict(text_template(tabular_input, window=1))
medication_texts_window2 = dict(text_template(tabular_input, window=2))
medication_texts_window3 = dict(text_template(tabular_input, window=3))
medication_texts_window4 = dict(text_template(tabular_input, window=4))
...
output_path = "data/llm_inputs/02_PONV/ponv_medications_text_template_4windows.jsonl"
...
print("Medication text templates (4 windows) for the PONV study cohort successfully saved.")