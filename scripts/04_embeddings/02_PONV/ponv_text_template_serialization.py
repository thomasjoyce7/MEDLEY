# Script to serialize tabular PONV medication data to a text format using a descriptive text template
    ...
    f"{possessive_pronoun.capitalize()} surgery was on {surgery_date_str}.")
        ...
        if group["medication"].isna().all() and group["admin_time"].isna().all():
            ...
            for _, row in group.iterrows():
                ...
                admin_hr = admin_time.strftime("%-I:%M %p") if pd.notnull(admin_time) else "an unknown hour"
                admin_day = admin_time.strftime("%B %-d, %Y") if pd.notnull(admin_time) else "an unknown day"
                ...
                entry = (f"At {admin_hr} on {admin_day}, {subject_pronoun} received {dose} {units} of {med} via {route} route.")
...
# Save as .jsonl
with open("data/llm_inputs/02_PONV/ponv_medications_text_template.jsonl", "w") as f:
    for subject_id, text in medication_texts:
        json.dump({"subject_id": subject_id, "text": text}, f)
        f.write("\n")
        
print("Medication text templates for the PONV study cohort successfully saved.")