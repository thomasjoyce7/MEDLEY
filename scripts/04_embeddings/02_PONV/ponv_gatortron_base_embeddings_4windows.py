# GatorTron-Base PONV application 
...
# GatorTron-base is BERT-like; cap at 512 tokens 
MAX_LEN = 512
...
# 
print("Calculating GatorTron-base embeddings (4 windows, normalized, concatenated).")
...
for input_path, template_name in input_paths.items():
    ...
    out_concat = os.path.join(
        out_dir, f"ponv_gatortron_base_{template_name}_concat4w_meanpool_l2norm.parquet"
    )
    ...
    for wname, mat in window_embs.items():
        out_w = os.path.join(
            out_dir, f"ponv_gatortron_base_{template_name}_{wname}_meanpool_l2norm.parquet"
        )
