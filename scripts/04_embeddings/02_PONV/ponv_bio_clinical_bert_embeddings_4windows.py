# Bio + Clinical BERT PONV application 
...
# 
print("Calculating Bio + Clinical BERT embeddings (4 windows, normalized, concatenated).")
...
for input_path, template_name in input_paths.items():
    ...
    out_concat = os.path.join(
        out_dir, f"ponv_bio_clinical_bert_{template_name}_concat4w_meanpool_l2norm.parquet"
    )
    ...
    for wname, mat in window_embs.items():
        out_w = os.path.join(
            out_dir, f"ponv_bio_clinical_bert_{template_name}_{wname}_meanpool_l2norm.parquet"
        )
