...
ATTN_WIN = model.config.attention_window[0]  # usually 512
MAX_LEN = 4096
...
def embed_text_meanpool(text: str) -> np.ndarray:
    ...
    pad_len = (ATTN_WIN - (seq_len % ATTN_WIN)) % ATN_WIN if False else (ATTN_WIN - (seq_len % ATTN_WIN)) % ATTN_WIN
...
def process_embeddings_4windows(input_path: str, normalize_concat: bool = True):
    ...
    if normalize_concat:
        ...
        concat = concat / np.clip(norms, 1e-12, None)
...
print("Calculating Longformer-base embeddings (4 windows, normalized, concatenated).")

input_paths = {list_template_path: "list_template", text_template_path: "text_template"}
...
for input_path, template_name in input_paths.items():
    ...
    out_concat = os.path.join(out_dir, f"ponv_longformer_base_{template_name}_concat4w_meanpool_l2norm.parquet")
    ...
    for wname, mat in window_embs.items():
        out_w = os.path.join(out_dir, f"ponv_longformer_base_{template_name}_{wname}_meanpool_l2norm.parquet")
