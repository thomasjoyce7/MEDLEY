# GatorTron-Base-2k PONV application 
...
# Concatenated embedding dimension = 1024*4 = 4096
...
# GatorTron-base-2k is BERT-like; cap at 2048 tokens
MAX_LEN = 2048
...
# 
print("Calculating GatorTron-base-2k embeddings (4 windows, normalized, concatenated).")
...
for input_path, template_name in input_paths.items():
    ...
    out_concat = os.path.join(
        out_dir, f"ponv_gatortron_base_2k_{template_name}_concat4w_meanpool_l2norm.parquet"
    )
    ...
    for wname, mat in window_embs.items():
        out_w = os.path.join(
            out_dir, f"ponv_gatortron_base_2k_{template_name}_{wname}_meanpool_l2norm.parquet"
        )
