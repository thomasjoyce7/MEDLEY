# Script to serialize tabular POD medication data to a text format using a descriptive text template
...
print("Converting tabular medication data to text format using text template.")
...
# Create text template function for tabular to text serialization -------------------------------------------------------

def text_template(tabular_input):
    text_outputs = []
    ...
    f"{possessive_pronoun.capitalize()} first positive CAM-ICU result occurred at {first_positive_str}.\n")
        ...
        if group["medication"].isna().all() and group["starttime"].isna().all():
            ...
            for _, row in group.iterrows():
                ...
                start_hr = start.strftime("%-I:%M %p") if pd.notnull(start) else "an unknown hour"
                start_day = start.strftime("%B %-d, %Y") if pd.notnull(start) else "an unknown day"
                stop_hr = stop.strftime("%-I:%M %p") if pd.notnull(stop) else "an unknown hour"
                stop_day = stop.strftime("%B %-d, %Y") if pd.notnull(stop) else "an unknown day"
                ...
                if start != stop:
                    if start.date() == stop.date():
                        # same day — only show stop hour
                        entry = (
                            f"From {start_hr} to {stop_hr} on {start_day}, {subject_pronoun} took {dose} {units} of {med} via {route} route."
                        )
                    else:
                        # different day — show stop date and stop hour
                        entry = (f"From {start_hr} on {start_day} to {stop_hr} on {stop_day}, {subject_pronoun} took {dose} {units} of {med} via {route} route."
                                )
                else:
                    # When start == stop, only display the start time
                    entry = (
                        f"At {start_hr} on {start_day}, {subject_pronoun} took {dose} {units} of {med} via {route} route."
                    )
...
medication_texts = text_template(tabular_input)

# Save as .jsonl
with open("data/llm_inputs/medications_text_template.jsonl", "w") as f:
    for subject_id, text in medication_texts:
        json.dump({"subject_id": subject_id, "text": text}, f)
        f.write("\n")

print("Medication text templates for POD positive patients successfully saved.")
