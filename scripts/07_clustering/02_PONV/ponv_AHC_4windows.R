# Agglomerative hierarchical clustering (AHC) for the PONV application
...
silhouette_plot_settings <- list(n_pcs=5, omit_first_pc = FALSE, center=TRUE, scale=FALSE, k_range=2:20)
...
silhouette_plot_output_path <- file.path(
  "figures/clustering/02_PONV/AHC",
  paste0("ponv_ahc_silhouette_grid_4windows_", silhouette_plot_suffix, ".pdf")
)

silhouette_plot_long_seq_encoders_output_path <- file.path(
  "figures/clustering/02_PONV/AHC",
  paste0("ponv_ahc_silhouette_grid_long_seq_encoders_4windows_", silhouette_plot_suffix, ".pdf")
)

clustering_suffix <- encode_ahc_clustering_settings(ahc_settings)
clustering_output_path <- file.path(
  "data/results/clustering/02_PONV/AHC",
  paste0("ponv_ahc_results_4windows_", clustering_suffix, ".csv")
)

dendrogram_output_path <- file.path(
  "figures/clustering/02_PONV/AHC",
  paste0("ponv_ahc_dendrogram_grid_4windows_", 
         ahc_settings$n_pcs, "pcs_",
         if (ahc_settings$center) "centered_" else "uncentered_",
         if (ahc_settings$scale) "scaled" else "unscaled", ".pdf")
)

dendrogram_long_seq_encoders_output_path <- file.path(
  "figures/clustering/02_PONV/AHC",
  paste0("ponv_ahc_dendrogram_grid_long_seq_encoders_4windows_", 
         ahc_settings$n_pcs, "pcs_",
         if (ahc_settings$center) "centered_" else "uncentered_",
         if (ahc_settings$scale) "scaled" else "unscaled", ".pdf")
)
...
if (run_silhouette_method == TRUE){
  ...
  # Grid layout: 
  silhouette_grid_long_seq_encoders <- 
  ...
  silhouette_grid_normal_seq_encoders <- 
    (silhouette_plots$bio_clin_bert_list | silhouette_plots$bio_clin_bert_text | silhouette_plots$bio_clin_bert_llm) /
    (silhouette_plots$gt_base_list | silhouette_plots$gt_base_text | silhouette_plots$gt_base_llm) / 
    (silhouette_plots$bert_list | silhouette_plots$bert_text | silhouette_plots$bert_llm)
  
  # Combine plots
  silhouette_grid_all <- 
    wrap_elements(silhouette_grid_long_seq_encoders) /
    wrap_elements(silhouette_grid_normal_seq_encoders) +
    plot_layout(nrow = 2, heights = c(3, 3))
  
  # Save plots to results 
  ggsave(silhouette_plot_output_path, silhouette_grid_all, width = 9, height = 12, units = "in", dpi = 300, device = cairo_pdf)
  
  # Save plots for long sequence encoders 
  ggsave(silhouette_plot_long_seq_encoders_output_path, silhouette_grid_long_seq_encoders, width = 9, height = 6, units = "in", dpi = 300, device = cairo_pdf)
...
if (run_dendrograms == TRUE){
  ...
  # Grid layout: 
  dendrogram_grid_long_seq_encoders <- 
  ...
  dendrogram_grid_normal_seq_encoders <- 
    (dendrogram_plots$bio_clin_bert_list | dendrogram_plots$bio_clin_bert_text | dendrogram_plots$bio_clin_bert_llm) /
    (dendrogram_plots$gt_base_list | dendrogram_plots$gt_base_text | dendrogram_plots$gt_base_llm) / 
    (dendrogram_plots$bert_list | dendrogram_plots$bert_text | dendrogram_plots$bert_llm)
  
  # Combine plots
  dendrogram_grid_all <- 
    wrap_elements(dendrogram_grid_long_seq_encoders) /
    wrap_elements(dendrogram_grid_normal_seq_encoders) +
    plot_layout(nrow = 2, heights = c(3, 3))
  
  # Save plots to results 
  ggsave(dendrogram_output_path, dendrogram_grid_all, width = 9, height = 12, units = "in", dpi = 300, device = cairo_pdf)
  
  # Save dendrograms for long sequence encoders
  ggsave(dendrogram_long_seq_encoders_output_path, dendrogram_grid_long_seq_encoders, width = 9, height = 6, units = "in", dpi =  300, device = cairo_pdf)
